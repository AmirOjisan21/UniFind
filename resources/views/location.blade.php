        ...
        <section class="bg-white dark:bg-gray-900 px-12">
            <div class="gap-16 items-center py-8 px-4 mx-auto max-w-screen-xl lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
                <div class="font-light text-gray-500 sm:text-lg dark:text-gray-400 px-12">
                    <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">{{$ksas->name}}</h2>
                    <p class="mb-4">{{$ksas->description}}</p>
                    <h3>Open Hours</h3>
                    <P>{{$ksas->open_hours}}</p>
                    <P>{{$ksas->important_details}}
                </div>
                <div class="grid grid-cols-2 gap-4 mt-8">
                    <img class="w-full rounded-lg" src="{{ asset('/images/' . $ksas->image) }}" alt="office content 1">
                </div>
            </div>
        </section>
        <div class="px-12">
        <a href="{{ route('ksaspub')}}"
